def main [
    ...
    let tags = if not ($env | get -o PLUGIN_TAGS | is-empty) {
        $tags | append ($env.PLUGIN_TAGS | split row ',' | str trim)
    } else if not ($env | get -o INPUT_TAGS | is-empty) {
    ...
    let tags = if (
        (not $no_github_tag)
         and
        (not ($env | get -o GITHUB_RUN_NUMBER | is-empty))
         and
        (not ($env | get -o GITHUB_SHA | is-empty))
    ) {
    ...
    let tags = if (
        (not $no_drone_tag)
         and
        (not ($env | get -o DRONE_BUILD_NUMBER | is-empty))
         and
        (not ($env | get -o DRONE_COMMIT_SHA | is-empty))
    ) {
    ...
    let auth = (
        if not ($username | is-empty) and ($password | is-empty) {
            ...
            (not ($env | get -o USERNAME | is-empty))
            and
            (not ($env | get -o PASSWORD | is-empty))
        ) {
            print "Got username and password from USERNAME and PASSWORD"
            {username: $env.USERNAME, password: $env.PASSWORD}
        } else if (
            (not ($env | get -o PLUGIN_USERNAME | is-empty))
            and
            (not ($env | get -o PLUGIN_PASSWORD | is-empty))
        ) {
            ...
            (not ($env | get -o INPUT_USERNAME | is-empty))
            and
            (not ($env | get -o INPUT_PASSWORD | is-empty))
        ) {
            ...
            (not ($env | get -o GITHUB_ACTOR | is-empty))
            and
            (not ($env | get -o GITHUB_TOKEN | is-empty))
        ) {
    ...
    let registry = (
        if ($registry | is-empty) {
            if not ($env | get -o PLUGIN_REGISTRY | is-empty) {
                $env.PLUGIN_REGISTRY
            } else if not ($env | get -o INPUT_REGISTRY | is-empty) {
                $env.INPUT_REGISTRY
            } else if not ($env | get -o REGISTRY | is-empty) {
                $env.REGISTRY
            } else if (
                (not ($env | get -o GITHUB_SERVER_URL | is-empty))
                and
                (not ($env | get -o GITHUB_ACTOR | is-empty))
            ) {
    ...
    let repository = (
        if ($repository | is-empty) {
            if not ($env | get -o PLUGIN_REPOSITORY | is-empty) {
                $env.PLUGIN_REPOSITORY
            } else if not ($env | get -o INPUT_REPOSITORY | is-empty) {
                $env.INPUT_REPOSITORY
            } else if not ($env | get -o REPOSITORY | is-empty) {
                $env.REPOSITORY
            } else {
